...
public class HairTest_Stage2_Male_Test extends BaseTest {
    ...
    @Test(description = "Verify Male User Can Fill All Details With Stage2", groups = {"regression"})
    @TestDescription("Verify the user can fill all the details with stage2 and proceed to payment")

    public void verifyMaleUserCanFillAllDetailsWithStage2() throws InterruptedException {
      ...
      //  Lead_You_Screen leadYouScreen = new Lead_You_Screen(DriverManager.getDriver());
        ...
        if(hairTestScreen.isHairLossTextDisplayed()==true) {
            ...
            Assert.assertTrue(hairTestScreen.isStage2TextOptionDisplayed(), "Stage 2 text option is not displayed");
            hairTestScreen.clickStage2Option();
